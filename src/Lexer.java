...
public class Lexer{
    ...
    private int _position, _line = 1, _column = 0;
    ...
    SyntaxToken findWhiteSpace(){
        char[] spaces = { ' ', '\f', '\r', '\t' };
        int start = _position;
        if (charIsInArray(getCurrentChar(), spaces)) {
            _position++; _column++;
            while (charIsInArray(getCurrentChar(), spaces)){
                _position++; _column++;
            }
    ...
    SyntaxToken findInteger(){
        ...
        int start  = _position;
        if (charIsInArray(getCurrentChar(), integers)) {
            _position++; _column++;
            while (charIsInArray(getCurrentChar(), integers)){
                _position++; _column++;
            }
